...
//用户上麦消费者
class ConsumeOnMicCommand extends Command
{
    const COMMAND_NAME = "ConsumeOnMicCommand";
    const QUEUE_NAME = "q_admin_user_room_onmic"; //队列名字
    const ROUTERKEY = ["OnMicEvent","LeaveMicEvent"]; //routerkey
    ...
    public  function callback($msg){
        ...
        if ($msg->body) {
            ...
            if ($body && isset($body['body'])) {
                ...
                $micid = $parseRes['micId'] ?? 0;
                $duration = $parseRes['duration'] ?? 0;
                $data = ["type" => $type, "room_id" => $room_id, "uid" => $uid,'ctime' => $ctime,'duration'=>$duration,"message_id"=>$message_id,"micid"=>$micid];
                Sharding::getInstance()->getConnectModel("bi","")->transaction(function()use($data){
                    $tableName = date('Ym',$data['ctime']);
                    ParseUserStateByUniqkey::getInstance()->insertOrUpdateModel(BiMessageOnmicModel::getInstance($tableName)->getModel(),[$data],["id","message_id"]);
                });
            }
