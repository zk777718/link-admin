...
class ImEmotionService
{
    ...
    {
        if (empty($where)) {
            $res = ImEmotionModel::getInstance()->getModel()->insertAll($data);
        } else {
            $res = ImEmotionModel::getInstance()->getModel()->where([$where])->update($data);
        }
        return $res;
    }
    ...
    public function getList($page, $where)
    {
        ...
        $count = ImEmotionModel::getInstance()->getModel()->where($where)->count();
