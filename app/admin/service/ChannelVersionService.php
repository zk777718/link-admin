...
class ChannelVersionService
{
    ...
    {
        if (empty($where)) {
            $res = ChannelVersionModel::getInstance()->getModel()->insertAll($data);
        } else {
            $res = ChannelVersionModel::getInstance()->getModel()->where([$where])->update($data);
        }
        return $res;
    }
    ...
    public function getList($page, $where)
    {
        ...
        $count = ChannelVersionModel::getInstance()->getModel()->where($where)->count();
        $list = ChannelVersionModel::getInstance()->getModel()
            ->where($where)
            ->order('id desc')
