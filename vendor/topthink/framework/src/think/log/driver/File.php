...
class File implements LogHandlerInterface
{
    ...
    public function __construct(App $app, $config = [])
    {
	 ...
	 //新添加log记录
        $self_request = $app->request->request();
        $self_server = $app->request->server();
        if ($app->request->host()) {
            $self_uri = $app->request->protocol() . ' ' . $app->request->method() . ' : ' . $app->request->url(true);
        } else {
            $self_uri = 'cmd:' . implode(' ', $_SERVER['argv']);
        }
        $pid = getmypid ();
        $ret1['debug'][] = $app->request->action().'---'.$app->request->header('token').'---进程id: '.$pid;
        $ret2['debug'][] = $app->request->action().'---'.$app->request->header('token').'---参数信息: '.json_encode(['请求地址: '=>$self_uri,'请求参数: '=>$self_request,'服务器信息: '=>$self_server]);
        $this->save($ret1);
        $this->save($ret2);
    }
